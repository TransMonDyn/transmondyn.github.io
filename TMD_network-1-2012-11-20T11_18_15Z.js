...
function init_new($divToUse, $typeReseau) { // TypeReseau as string
	...
	var tmpLegendNetwork = init_legend($typeReseau, 'bl_legende_content', mainNetwork);
...
function dyn_displaySelectionDescription($networkSelection, $typeNetwork, $nodesTable, $contextDiv){
	...
	if ($networkSelection != ""){
		for (var i = 0; i < $networkSelection.length; i++){
			...
			{
				case 'axe':
					selectionHTML += '<a href="' + axeHTMLurl + urlToEncode + '">' + descTextToEncode + '</a><br>';
					break;
				case 'membre':
					selectionHTML += '<a href="' + memberHTMLurl + urlToEncode + '">' + descTextToEncode + '</a><br>';
					break;
				case 'transition':
					selectionHTML += '<a href="' + transitionHTMLurl + urlToEncode + '">' + $nodesTable.getValue($networkSelection[i].row, nodesTextColumn) + " : " + descTextToEncode + '</a><br>';
					break;
				default:
					break;
			};		
		...
		document.getElementById($contextDiv).innerHTML = "No item currently selected..."
	};
};
...
function init_legend($typeReseau, $legendContentDiv, $mainNetwork){
	...
	myLegendNodes = legend_setXY(legend_options, myLegendNodes);
...
function legend_setXY($optionObject, $nodesTable){
